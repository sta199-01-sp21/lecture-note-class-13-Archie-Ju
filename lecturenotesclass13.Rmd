...
```{r}
library (usethis)
use_git_config (user.name = "Archie-ju", user.email = "botao.ju@duke.edu")
```
...
```{r data_manhattan}
manhattan <- read_csv("~/R/Lecture Notes/Lecture-Notes-13/manhattan.csv")
```
...
population <- tibble(act = rnorm(n = 1000, mean = 20.8, sd = 5.8)) # generating a normal distribution based on mean and sd.
population %>% 
  ggplot(aes(x = act)) +
...
* we have to assume that the 20 observations in our sample is fairly representative of the population
First, identify the following:

- Population: all 1-bedroom apartments in Manhattan
- Parameter of interest: mean price of such appartments
- Sample: we have sample of 20 one-bedroom apartment
- Sample size: 20
...
1. **Take a bootstrap sample** - a random sample taken with replacement 
   from the original sample, of the same size as the original sample.
      sample of 20, same as original sample; with replacement
...
boot_medians
```
My interpretation: We are 95% confident that the true median price of one-bed apartment in Manhattan is between 2162 to 2875 price interval.
...
- Population: all mice under radiation
- Parameter of interest: proportion of mice died while not on treatment
...
boot_prop <- mice %>% 
  filter(treat == 0) %>% 
  mutate(died = if_else(died == 1, "yes", "no")) %>% # if die equals 1, die becomes yes, otherwise no
  specify(response = died, success = "yes") %>% 
  generate(reps = 10000, type = "bootstrap") %>% 
  calculate(stat = "prop") # calculate proportion for each replicate of 262 items
boot_prop
```
...
What does this mean?
We are 95% confident that 0.690 - 0.797 captures the true proportion.
**Interpretation**:
...
```{r practice_2_ci}
ci_mice_died_2 <- get_ci(boot_prop, level = .95)
```
